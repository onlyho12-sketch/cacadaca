...
DEFAULT_CKPT = os.path.join(_REPO, "learning", "rl", "champion", "model_bc.pt")
...
OUTPUT_COLUMNS = [
    ...
    "process_time_s", "episode_completed",
]

EVALUATION_MODE = "flat_trained_bc_policy_inference"   # 곡면 재학습/검증 아님 (README 7장)
...
def run_cell_episode(row: dict, recipe: Recipe, policy, cal) -> dict:
    """셀 하나: patch 합성 → BC 정책으로 에피소드 실행 → 전/후 품질 + 판정."""
    ...
    obs_cc_margin = 20.0
    a_sum = np.zeros(2); f_sum = 0.0; feed_sum = 0.0
    hard_violated = False
    completed = False
...
def main():
    ...
    print(f"[vehicle] 입력 {len(rows)} 셀 | 정책 BC({os.path.basename(args.checkpoint)}) "
          f"| clearcoat 안전기준 {CLEARCOAT_SAFE_MIN_UM:.0f} μm | mode={EVALUATION_MODE}")

    if args.workers > 1:
        import multiprocessing as mp
        with mp.get_context("fork").Pool(args.workers, _init_worker,
                                         (args.checkpoint,)) as pool:
            results = pool.map(_work, rows, chunksize=2)
    else:
        _init_worker(args.checkpoint)
    ...
    summary = {
        "policy_type": "BC (behavior cloning of handmade dwell policy) — PPO 아님",
