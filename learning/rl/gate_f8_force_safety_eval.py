"""Paired Gate F8 evaluation of base14 force-action safety constraints."""
...
parser.add_argument("--shield-mode", choices=("control", "static_cap", "predictive_shield"), required=True)
...
from learning.rl.gate_f8_force_safety import apply_force_action_shield  # noqa: E402
...
def main() -> None:
    ...
    try:
        for step in range(args.max_control_steps):
            ...
            executed_np, caps = apply_force_action_shield(
                parent_action.detach().cpu().numpy(), full.detach().cpu().numpy(),
                args.shield_mode, curved=args.surface_kind != "flat")
            ...
            for i in active:
                ...
                shield_steps[i] += int(executed_np[i, 0] < p[0] - 1e-7)
                control_steps[i] += 1
            ...
            for i in (terminated | truncated).nonzero(as_tuple=False).squeeze(-1).cpu().tolist():
                ...
                sequence_rows.append({
                    ...
                    **latched, "control_steps": n, "contact_steps": int(contact_steps[i]),
                    ...
                    "executed_feed_action_mean": float(executed_sum[i, 1] / n),
                ...
                active.remove(i)
            if (step + 1) % 1000 == 0:
                print(f"[Gate F8] {args.shield_mode}/{args.surface_kind}/{args.direction_mode} "
                      f"step={step + 1} complete={env.num_envs-len(active)}/{env.num_envs}", flush=True)
        if args.smoke_only:
            smoke = {
                "created_utc": datetime.now(timezone.utc).isoformat(),
                "gate": "F8_FORCE_SAFETY_SMOKE",
        ...
        metadata = {
            "created_utc": datetime.now(timezone.utc).isoformat(), "gate": "F8_FORCE_SAFETY",
            "shield_mode": args.shield_mode, "surface_kind": args.surface_kind,
