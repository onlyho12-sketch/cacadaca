"""Gate F10-A: substep-resolved trace of Gate F9 curved force overloads.
...
parser.add_argument("--shield-mode", choices=("control", "static_cap"), required=True)
...
def main() -> None:
    ...
    try:
        for step in range(args.max_control_steps):
            ...
            executed_np, _caps = apply_force_action_shield(
                parent_np, full.detach().cpu().numpy(), args.shield_mode,
                curved=args.surface_kind != "flat")
            ...
            for i in (terminated | truncated).nonzero(as_tuple=False).squeeze(-1).cpu().tolist():
                active.discard(i)
            if (step + 1) % 200 == 0:
                print(f"[F10A] {args.shield_mode}/{args.surface_kind}/{args.direction_mode}/"
        ...
        for r in rows:
            r["shield_mode"] = args.shield_mode
        ...
        meta = {
            ...
            "gate": "F10A_OVERLOAD_SUBSTEP_TRACE",
