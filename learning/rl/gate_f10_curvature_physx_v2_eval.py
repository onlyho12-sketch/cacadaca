"""Gate F10-G2 one-sided-slew predictive curvature PhysX pilot (no training)."""
...
parser.add_argument("--shield-mode", choices=("control", "curvature_safety_v2"), required=True)
...
def main() -> None:
    ...
    parent_contract_all = np.ones(env.num_envs, dtype=bool)
    predictive_cap_sum = np.zeros(env.num_envs); predictive_cap_min = np.ones(env.num_envs)
    ...
    try:
        for step in range(args.max_control_steps):
            ...
            for i in active:
                ...
                parent_contract_all[i] &= bool(np.all(executed_np[i] <= p + 1e-7))
                predictive_cap_sum[i] += float(shield["predictive_force_cap_action"][i])
                predictive_cap_min[i] = min(predictive_cap_min[i], float(
                    shield["predictive_force_cap_action"][i]))
                control_steps[i] += 1
            ...
            for i in (terminated | truncated).nonzero(as_tuple=False).squeeze(-1).cpu().tolist():
                ...
                sequence_rows.append({
                    ...
                    **latched, "control_steps": n, "contact_steps": int(contact_steps[i]),
                    ...
                    "parent_upper_bound_contract": bool(parent_contract_all[i]),
                    "predictive_force_cap_mean": float(predictive_cap_sum[i] / n),
                    "predictive_force_cap_min": float(predictive_cap_min[i]),
        ...
        if args.smoke_only:
            smoke = {
                ...
                "gate": "F10G2_CURVATURE_SAFETY_V2_SMOKE",
        ...
        metadata = {
            "created_utc": datetime.now(timezone.utc).isoformat(), "gate": "F10G2_CURVATURE_SAFETY_V2",
            "shield_mode": args.shield_mode, "surface_kind": args.surface_kind,
            ...
            "safety_interface": "F10_CURVATURE_SAFETY_V2",
