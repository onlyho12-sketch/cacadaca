"""Gate C2 one-candidate paired-seed PhysX evaluator (no training)."""
...
parser.add_argument("--surface_profile", choices=("legacy_stress", "new_car_mild"), required=True)
...
from learning.rl.env.gate_c_planar_roi_polish_env import GateCPlanarRoiPolishEnv  # noqa: E402
from learning.rl.env.gate_c_planar_roi_polish_env_cfg import GateCPlanarRoiPolishEnvCfg  # noqa: E402
...
def main() -> None:
    ...
    cfg.gate_c_step_over_ratio = args.step_over_ratio
    cfg.gate_c_edge_mode = args.edge_mode
    cfg.gate_c_direction_mode = args.direction_mode
    ...
    env = GateCPlanarRoiPolishEnv(cfg, render_mode=None)
    ...
    for env_id in range(args.num_envs):
        ...
        tile_rows.extend(_tile_rows(env_id, "initial", 0, diagnostic["tile_maps"]))
    ...
    while len(completed) < args.num_envs and step < args.max_control_steps:
        ...
        for env_id in done_ids:
            ...
            sequence_rows.append({
                ...
                "step_over_ratio": args.step_over_ratio,
                "edge_mode": args.edge_mode,
                "checkpoint": os.path.basename(checkpoint), "env": env_id,
                "profile_seed": initial_rows[env_id]["profile_seed"],
            ...
            for item in log.get("pass_history", []):
                row = {
                    ...
                    "edge_mode": args.edge_mode,
                    "checkpoint": os.path.basename(checkpoint), "env": env_id,
                    "profile_seed": initial_rows[env_id]["profile_seed"], **item,
                }
        ...
        if step % 1000 == 0:
            print(f"[Gate C2] step={step} complete={len(completed)}/{args.num_envs}", flush=True)
    ...
    metadata = {
        ...
        "surface_profile": args.surface_profile,
        "candidate": dict(env.gate_c_path_metadata),
        "num_envs": args.num_envs, "max_passes": 1,
        "physical_contact": True, "feed_speed_mm_s": 12.7,
        "steps_executed": step, "complete": len(completed) == args.num_envs,
        "profile_seeds": [int(row["profile_seed"]) for row in initial_rows],
        ...
        "caveat": "new_car_mild distributions and mar optics are PT-DESIGN.",
    }
    with open(os.path.join(out_dir, "metadata.json"), "w", encoding="utf-8") as handle:
        json.dump(metadata, handle, indent=2, sort_keys=True)
    print(f"[Gate C2] wrote {out_dir}")
    print(f"[Gate C2] complete={len(completed)}/{args.num_envs} steps={step}")
    env.close()
    if len(completed) != args.num_envs:
        raise RuntimeError(f"incomplete Gate C2 run: {len(completed)}/{args.num_envs}")
